...
# https://github.com/yexiang92/opstool/blob/v1.0.19/opstool/post/eigen_data.py

# The `ops.wipeAnalysis()` function has been commented out to prevent uncontrolled wipe analysis.
# Instead, wipe analysis will be controlled through the `prepare_next_analysis_stage` in the p008_05_model_analysis.py.
...
from opstool.utils import CONFIGS, get_random_color
from opstool.post.model_data import GetFEMData
...
def _get_modal_properties(mode_tag: int = 1, solver: str = "-genBandArpack"):
    ...
    """
    # ops.wipeAnalysis() # has been commented out to prevent uncontrolled wipe analysis.
    if mode_tag == 1:
        ops.eigen(solver, 2)
    else:
